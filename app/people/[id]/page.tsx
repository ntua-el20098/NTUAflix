      ...
      try {
        ...
        if (data.nameObject.nameTitles && data.nameObject.nameTitles.length > 0) {
          ...
          const movieDetailsPromises = data.nameObject.nameTitles.map(async (title) => {
            ...
            if (titleResponse.ok) {
              const titleData: { titleObject: { titlePoster: string; originalTitle: string } } =
            ...
            <div className="space-y-6">
              ...
              <p className="text-lg">
                ...
                {personData.profession.split(",").map((prof, index) => (
                  <span key={index} style={{ marginLeft: '6px' }} className="badge bg-secondary">
                    {prof.trim()}
                  </span>
                ))}
              </p>
                ...
                <div className="cast-card-container flex items-center ">
                  {movieDetails.map((movie, index) => (
                    <MovieCard
                      ...
                      image={
                        movie.titlePoster &&
                        movie.titlePoster.replace("{width_variable}", "original") ||
                        fallbackPosterUrl
                      }
                    />
                  ))}
